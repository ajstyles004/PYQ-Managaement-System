...
<body class="container py-4">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div>
      ...
      <span class="me-2 small-muted">Logged in as: <strong><?php echo htmlspecialchars($_SESSION['admin_username'] ?? ''); ?></strong></span>
        <!-- Create Admin / Create User buttons removed per request -->
      <a class="btn btn-outline-danger" href="admin_logout.php">Logout</a>
    </div>
  </div>
  ...
  <div class="card mb-4">
    <div class="card-header d-flex justify-content-between align-items-center">
      ...
      <div>
        <select id="manageDeptFilter" class="form-select d-inline-block" style="width:220px">
        ...
        <select id="manageStudyYearFilter" class="form-select d-inline-block ms-2" style="width:140px">
    ...
    <div class="card-body">
      ...
      <div class="table-responsive">
        <table class="table table-striped" id="subTable">
          <thead><tr><th>ID</th><th>Code</th><th>Name</th><th>Dept</th><th>Sem</th><th>Year</th><th>Actions</th></tr></thead>
          <tbody></tbody>
        </table>
      </div>
    </div>
  ...
  <div class="card mb-4" id="papersCard">
    <div class="card-header d-flex justify-content-between align-items-center">
      ...
      <div>
        <select id="papersDeptFilter" class="form-select d-inline-block" style="width:180px">
          <option value="">Filter: All Departments</option>
          <?php foreach ($departments as $d): ?>
            <option value="<?=htmlspecialchars($d['department_id'])?>"><?=htmlspecialchars($d['department_name'])?></option>
          <?php endforeach; ?>
        ...
        <select id="papersSubFilter" class="form-select d-inline-block ms-2" style="width:240px">
          <option value="">Filter: All Subjects</option>
        </select>

        <select id="papersStudyYear" class="form-select d-inline-block ms-2" style="width:120px">
          <option value="">All Years</option>
          <option value="1">1st</option>
          <option value="2">2nd</option>
          <option value="3">3rd</option>
        ...
        <select id="papersSemester" class="form-select d-inline-block ms-2" style="width:120px">
        ...
        <input id="papersQ" class="form-control d-inline-block ms-2" placeholder="Search subject code (e.g. CS101)" style="width:180px">
    ...
    <div class="card-body">
      ...
      <div class="table-responsive">
        <table class="table table-striped" id="papersTableAdmin">
          <thead><tr><th>ID</th><th>Title</th><th>Dept</th><th>Subject</th><th>Code</th><th>Year</th><th>File</th><th>Actions</th></tr></thead>
          <tbody></tbody>
        </table>
      </div>
    </div>
...
function loadSubs(deptId = null){
  const studyYear = $('#manageStudyYearFilter') ? $('#manageStudyYearFilter').value : '';
  ...
  fetch(url).then(r=>r.json()).then(data=>{
    ...
    data.forEach(s=>{
      const year = s.semester ? Math.ceil(Number(s.semester)/2) : '';
      const tr = document.createElement('tr');
...
$('#manageStudyYearFilter').addEventListener('change', ()=> loadSubs($('#manageDeptFilter').value || null));
...
$('#papersDeptFilter').addEventListener('change', ()=>{
  ...
  const studyYear = $('#papersStudyYear').value;
  let url = 'subject_api.php?action=list&department=' + encodeURIComponent(dept);
  if (studyYear) url += '&study_year='+encodeURIComponent(studyYear);
  fetch(url).then(r=>r.json()).then(list=>{ sel.innerHTML = '<option value="">All Subjects</option>'; list.forEach(s=>{ const o = document.createElement('option'); o.value=s.subject_id; o.textContent=s.subject_name + (s.subject_code?(' ('+s.subject_code+')'):''); sel.appendChild(o); }); loadPapers(); }).catch(()=> sel.innerHTML = '<option value="">Error</option>');
});
$('#papersStudyYear').addEventListener('change', ()=> { // when study year changed, reload subjects under selected dept
  $('#papersDeptFilter').dispatchEvent(new Event('change'));
});
...
function loadPapers(){
  ...
  const dept = $('#papersDeptFilter').value;
  const studyYear = $('#papersStudyYear').value;
  const semester = $('#papersSemester').value;
  const q = $('#papersQ').value.trim();
  ...
  const tbody = $('#papersTableAdmin tbody'); tbody.innerHTML = '<tr><td colspan="8">Loading...</td></tr>';
  fetch(url).then(r=>r.json()).then(data=>{
    tbody.innerHTML = '';
    if (!Array.isArray(data) || data.length===0) { tbody.innerHTML = '<tr><td colspan="8">No papers found.</td></tr>'; return; }
    data.forEach(p=>{
      const tr = document.createElement('tr');
...
// wire up papers search input 'Enter' key
$('#papersQ').addEventListener('keydown', function(e){ if (e.key === 'Enter') { e.preventDefault(); loadPapers(); } });
