...
try {
    ...
    if ($action === 'list') {
        ...
        $semester_q = isset($_GET['semester']) && $_GET['semester'] !== '' ? intval($_GET['semester']) : null;
        $q = isset($_GET['q']) ? trim($_GET['q']) : '';
        $sql = "SELECT p.paper_id, p.title, p.year, p.semester, p.exam_type, p.filename, p.filepath,
                       p.department_id, p.subject_id, d.department_name, s.subject_name, s.subject_code
                FROM paper p
        ...
        // map study_year to semester range
        if ($study_year !== null) {
            $startSem = ($study_year - 1) * 2 + 1;
            $endSem = $study_year * 2;
            $sql .= " AND p.semester BETWEEN ? AND ?"; $params[] = $startSem; $params[] = $endSem; $types .= 'ii';
        }
        if ($semester_q !== null) { $sql .= " AND p.semester = ?"; $params[] = $semester_q; $types .= 'i'; }
