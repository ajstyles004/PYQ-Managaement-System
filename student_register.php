...
// Handle explicit cancel/back action to return to registration form
if (isset($_GET['action']) && $_GET['action'] === 'cancel_registration') {
  unset($_SESSION['register_step'], $_SESSION['register_email'], $_SESSION['register_student_id'], $_SESSION['register_sent_at']);
  // Redirect to same page without query string to show fresh registration form
  ob_end_clean();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'register') {
        ...
        if ($email === '' || $password === '') {
            ...
            $chk = $mysqli->prepare('SELECT student_id, is_verified FROM student_user WHERE email = ? LIMIT 1');
            ...
            if ($exists && $exists['is_verified']) {
              ...
              // If an unverified record exists, update it instead of inserting a duplicate
              ...
              if ($exists && !$exists['is_verified']) {
                ...
                $upd = $mysqli->prepare('UPDATE student_user SET username = ?, full_name = ?, password_hash = ?, is_verified = 0 WHERE student_id = ? LIMIT 1');
                if ($upd) {
                  $upd->bind_param('sssi', $username, $full_name, $hash, $student_id);
                  if (!$upd->execute()) {
                    $error = 'Registration update failed: ' . $mysqli->error;
                    $upd->close();
                ...
                $stmt = $mysqli->prepare('INSERT INTO student_user (username, email, full_name, password_hash, is_verified) VALUES (?, ?, ?, ?, 0)');
                if ($stmt) {
              ...
              // If no error so far, generate/send OTP for the (new or updated) record
              if (!$error) {
                $otp = generate_otp();
                ...
                if ($result['success']) {
                  ...
                  $_SESSION['register_sent_at'] = time();
                  $message = 'Account created! OTP sent to your email for verification.';
                  $step = 2;
                } else {
                  // If we inserted a new record and email sending failed, remove it
                  if (isset($student_id) && empty($exists)) {
        ...
        <div class="card-body">
          ...
          <form method="post" action="student_register.php">
            ...
            <div class="text-center">
              ...
              <p class="text-muted">
                <a href="student_register.php?action=cancel_registration" class="btn btn-link btn-sm">← Back to Registration</a>
              </p>
            </div>
          </form>
          <?php endif; ?>
